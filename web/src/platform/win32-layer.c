...
// ====== Web build: Android-specific globals removed ======
// extern JavaVM *java_machine;            -> no JVM in browser
// extern jobject bitmapMainScreen;        -> SDL_Texture is used instead (in web-emu.c)
// extern AndroidBitmapInfo androidBitmapInfo;
// AAssetManager *assetManager;            -> assets read via fopen() from preloaded FS
// =========================================================
...
HANDLE CreateFile(LPCTSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
	...
	if (!forceNormalFile && !foundDocumentScheme && !urlContentSchemeFound) {
		// Web build: assets are mounted at "/assets/" in the Emscripten
		// virtual filesystem via --preload-file. We try several path
		// variations so includes inside KML (e.g. "keyb4950.kmi") and
		// bare ROM names (e.g. "rom.50g") also resolve.
		int fd = -1;
		TCHAR szTry[MAX_PATH];

		// Variation 1: as given
		fd = open(lpFileName, O_RDONLY);

		// Variation 2: with current asset directory prepended (if set)
		if (fd < 0 && szCurrentAssetDirectory) {
			_tcscpy(szTry, szCurrentAssetDirectory);
			_tcscat(szTry, lpFileName);
			fd = open(szTry, O_RDONLY);
		}

		// Variation 3: prefix with "/assets/" (absolute Emscripten FS path)
		if (fd < 0) {
			_tcscpy(szTry, _T("/assets/"));
			_tcscat(szTry, lpFileName);
			fd = open(szTry, O_RDONLY);
		}

		// Variation 4: prefix with "assets/" (relative)
		if (fd < 0) {
			_tcscpy(szTry, _T("assets/"));
			_tcscat(szTry, lpFileName);
			fd = open(szTry, O_RDONLY);
		...
		// Variation 5: if path already starts with "assets/", try with leading slash
		if (fd < 0 && _tcsncmp(lpFileName, assetsPrefix, assetsPrefixLength) == 0) {
			szTry[0] = _T('/');
			_tcscpy(szTry + 1, lpFileName);
			fd = open(szTry, O_RDONLY);
		}

		if (fd >= 0) {
			LOGD("CreateFile() opened '%s' (fd=%d)", lpFileName, fd);
			HANDLE handle = malloc(sizeof(struct _HANDLE));
			memset(handle, 0, sizeof(struct _HANDLE));
			handle->handleType = HANDLE_TYPE_FILE;
			handle->fileDescriptor = fd;
			handle->fileAsset = NULL;
			return handle;
		}
		LOGE("CreateFile() FAILED to open '%s' under any prefix", lpFileName);
	} else {
...
ReadFile(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead,
	...
	if (hFile->handleType == HANDLE_TYPE_FILE) {
		...
		// Web build: no AAsset; this branch should never trigger because
		// the asset path in CreateFile() now produces HANDLE_TYPE_FILE.
		readByteCount = 0;
...
if (hFile->handleType == HANDLE_TYPE_FILE && nNumberOfBytesToRead > 1000) {
		LOGD("ReadFile LARGE: fd=%d requested=%u read=%u",
		     hFile->fileDescriptor, nNumberOfBytesToRead, readByteCount);
	}
...
SetFilePointer(HANDLE hFile, LONG lDistanceToMove, PLONG lpDistanceToMoveHigh, DWORD dwMoveMethod) {
	...
	if (hFile->handleType == HANDLE_TYPE_FILE) {
		...
		seekResult = -1; // Web build: HANDLE_TYPE_FILE_ASSET no longer produced
	}
...
DWORD GetFileSize(HANDLE hFile, LPDWORD lpFileSizeHigh) {
	...
	if (hFile->handleType == HANDLE_TYPE_FILE) {
		...
		return 0; // Web build: HANDLE_TYPE_FILE_ASSET no longer produced
	}
	return 0;
}
...
LPVOID MapViewOfFile(HANDLE hFileMappingObject, DWORD dwDesiredAccess, DWORD dwFileOffsetHigh,
	...
	if (hFileMappingObject->handleType == HANDLE_TYPE_FILE_MAPPING) {
		...
		if (dwDesiredAccess & FILE_MAP_WRITE)
			return NULL;
		// Web build: HANDLE_TYPE_FILE_MAPPING_ASSET no longer produced
		hFileMappingObject->fileMappingAddress = NULL;
	}
...
// Web build: no JVM to detach from. Stub provided so the rest of the code
// compiles without changes.
static inline int jniDetachCurrentThread(void) { return 0; }
...
BOOL WINAPI CloseHandle(HANDLE hObject) {
	...
	switch (hObject->handleType) {
		...
		case HANDLE_TYPE_FILE_ASSET: {
			...
			// Web build: AAsset no longer used; just free the handle
			hObject->handleType = HANDLE_TYPE_INVALID;
...
// ====================================================================
// Wave API — Web build stub
// --------------------------------------------------------------------
// Audio is intentionally stubbed in this initial port. The original
// Android implementation used OpenSL ES with two engines selectable via
// NEW_WIN32_SOUND_ENGINE. We replace all entry points with no-ops so
// the emulator core links and runs silently. Audio can be re-added later
// using SDL_mixer or the Web Audio API.
// ====================================================================

MMRESULT waveOutOpen(LPHWAVEOUT phwo, UINT uDeviceID, LPCWAVEFORMATEX pwfx,
                     DWORD_PTR dwCallback, DWORD_PTR dwCallbackInstance, DWORD fdwOpen) {
	(void)phwo; (void)uDeviceID; (void)pwfx;
	(void)dwCallback; (void)dwCallbackInstance; (void)fdwOpen;
	if (phwo) *phwo = NULL;
	return MMSYSERR_NOERROR;
}

MMRESULT waveOutReset(HWAVEOUT hwo) { (void)hwo; return MMSYSERR_NOERROR; }
MMRESULT waveOutClose(HWAVEOUT handle) { (void)handle; return MMSYSERR_NOERROR; }
...
BOOL PatBlt(HDC hdcDest, int x, int y, int w, int h, DWORD rop) {
	...
	if ((hdcDest->selectedBitmap || hdcDest->hdcCompatible == NULL) && w && h) {
		...
		if (hdcDest->hdcCompatible == NULL) {
			extern void *webMainScreenPixels;
			extern int   webMainScreenWidth;
			extern int   webMainScreenHeight;
			extern int   webMainScreenStride;
			destinationWidth  = webMainScreenWidth;
			destinationHeight = webMainScreenHeight;
			destinationStride = webMainScreenStride;
			pixelsDestination = webMainScreenPixels;
			if (!pixelsDestination) {
				return FALSE;
			}
		} else {
		...
		COLORREF brushColor = 0xFF000000;
			...
			for (int currentX = x; currentX < x + w; currentX++) {
				...
				switch (rop) {
					...
					case BLACKNESS:
						...
						break;
					case WHITENESS:
						destinationPixel[0] = 255;
						destinationPixel[1] = 255;
						destinationPixel[2] = 255;
						destinationPixel[3] = 255;
						break;
					case PATCOPY:
...
StretchBlt(HDC hdcDest, int xDest, int yDest, int wDest, int hDest, HDC hdcSrc, int xSrc, int ySrc,
	...
	if (hdcDest && hdcSrc
		...
		// JNIEnv removed in web build.
		void *jniEnv = NULL;
		int ret = 0; (void)ret;

static int s_bitblt_total = 0;
		s_bitblt_total++;
		if (s_bitblt_total <= 5) {
			LOGD("BitBlt entry #%d: hdcDest=%p hdcDest->hdcCompatible=%p",
			     s_bitblt_total, hdcDest,
			     hdcDest ? (void*)hdcDest->hdcCompatible : (void*)0);
		}

	extern int g_inOnPaint;
	if (hdcDest->hdcCompatible == NULL) {
...
// DIAGNOSTIC: log a few bytes from the source bitmap to know if it
		// actually contains image data or is all zero.
		if (hdcDest->hdcCompatible == NULL) {
			static int s_src_log = 0;
			s_src_log++;
			if (s_src_log <= 5 && pixelsSource && sourceWidth > 0 && sourceHeight > 0) {
				unsigned char *sp = (unsigned char *)pixelsSource;
				// pega pixels em quatro posições diferentes da source
				int p0 = 0;
				int p1 = (sourceHeight / 4) * sourceStride + (sourceWidth / 4) * 4;
				int p2 = (sourceHeight / 2) * sourceStride + (sourceWidth / 2) * 4;
				int p3 = (3*sourceHeight / 4) * sourceStride + (3*sourceWidth / 4) * 4;
				LOGD("StretchBlt SRC bytes #%d: srcW=%d srcH=%d srcBitCount=%d "
				     "[0,0]=%02X%02X%02X%02X "
				     "[1/4,1/4]=%02X%02X%02X%02X "
				     "[1/2,1/2]=%02X%02X%02X%02X "
				     "[3/4,3/4]=%02X%02X%02X%02X",
				     s_src_log, sourceWidth, sourceHeight, sourceBitCount,
				     sp[p0+0], sp[p0+1], sp[p0+2], sp[p0+3],
				     sp[p1+0], sp[p1+1], sp[p1+2], sp[p1+3],
				     sp[p2+0], sp[p2+1], sp[p2+2], sp[p2+3],
				     sp[p3+0], sp[p3+1], sp[p3+2], sp[p3+3]);
			}
		}
		...
		// Web build: nothing to unlock (no JNI bitmap).
		(void)jniEnv;
...
HDC BeginPaint(HWND hWnd, LPPAINTSTRUCT lpPaint) {
    if (lpPaint) {
        ...
        lpPaint->fErase = TRUE;
        lpPaint->hdc = CreateCompatibleDC(NULL);
        lpPaint->rcPaint.right = (short) nBackgroundW;
        lpPaint->rcPaint.bottom = (short) nBackgroundH;

        static int s_beginpaint_ctr = 0;
        s_beginpaint_ctr++;
        if (s_beginpaint_ctr <= 5) {
            LOGD("BeginPaint #%d: hdc=%p rcPaint=(0,0,%d,%d) hdcCompatible=%p",
                 s_beginpaint_ctr, (void*)lpPaint->hdc,
                 (int)lpPaint->rcPaint.right, (int)lpPaint->rcPaint.bottom,
                 (void*)lpPaint->hdc->hdcCompatible);
        }
...
// web_pump_timers
// ---------------------------------------------------------------------
// Single-threaded replacement for timerThreadStart. Emscripten + Asyncify
// has no real pthreads, so timers cannot run on a background thread.
// Instead we drive them from the CPU yield: web_yield_to_browser() calls
// this function once per yield, and any timer whose triggerTime has
// elapsed fires its callback. Periodic timers are rescheduled in place.
// =====================================================================
void web_pump_timers(void) {
    pthread_mutex_lock(&timerEventsLock);

static int s_pump_ctr = 0;
	s_pump_ctr++;
	if (s_pump_ctr <= 5 || (s_pump_ctr % 2000) == 0) {
		int cnt = 0;
		for (struct timerEvent *t = timersList.next; t != &timersList; t = t->next) cnt++;
		LOGD("web_pump_timers #%d: %d timer(s) in list", s_pump_ctr, cnt);
	}


    ULONGLONG now = GetTickCount64();
    int safety = 64;  // hard cap per pump to avoid runaway loops
    while (safety-- > 0) {
        struct timerEvent *timer =
            (timersList.next == &timersList ? NULL : timersList.next);
        if (!timer) break;
        if (timer->triggerTime > now) break;

        // remove from list
        timer->next->prev = timer->prev;
        timer->prev->next = timer->next;

        struct timerEvent *toFree;
        if (timer->fuEvent == TIME_PERIODIC) {
            timer->triggerTime += timer->uDelay;
            insertTimer(timer);
            toFree = NULL;
        ...
        LPTIMECALLBACK fptc    = timer->fptc;
        DWORD_PTR      dwUser  = timer->dwUser;
        ULONGLONG      tt      = timer->triggerTime;

        {
            static unsigned int s_fire_ctr = 0;
            s_fire_ctr++;
            if (s_fire_ctr <= 30 || (s_fire_ctr % 256) == 0) {
                LOGD("pump fire #%u: timerId=%d dwUser=%lu triggerTime=%llu now=%llu (late=%lldms)",
                     s_fire_ctr, timerId, (unsigned long)dwUser,
                     tt, now, (long long)(now - tt));
            }
        }
...
LOGD("timeSetEvent CALLED: uDelay=%u uResolution=%u fuEvent=%u dwUser=%lu",
	     uDelay, uResolution, fuEvent, (unsigned long)dwUser);



if (!timerThreadId) {
#ifdef __EMSCRIPTEN__
		// No real pthreads here. Timers are pumped from web_yield_to_browser
		// via web_pump_timers(). Mark thread "alive" so we don't try again.
		timerThreadId = (pthread_t) 1;
		LOGD("timeSetEvent: web build, timer pump active (no pthread)");
#else
...
MMRESULT timeGetDevCaps(LPTIMECAPS ptc, UINT cbtc) {
	if (ptc) {
		ptc->wPeriodMin = 1; // ms
		// IMPORTANT: Web build is single-threaded. The original Win32 cap
// of 1000000 ms (1000s) is catastrophic here because a single
// over-range timer2 callback blocks the entire emulator that long.
// Keep this small so web_pump_timers gets a chance to drive the
// Saturn timer2 forward at human-perceptible rates.
ptc->wPeriodMax = 100;   // ms (was 1000000)
	}
	return 0; //No error
}
