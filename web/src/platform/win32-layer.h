...
// ====== Emscripten/Web platform substitutes for Android headers ======
// Original Android-only includes (<jni.h>, <android/bitmap.h>,
// <android/asset_manager.h>, <android/log.h>) removed for web build.
// We use stdio/Emscripten logging instead. AAsset is no longer used;
// assets are read via standard fopen() from the preloaded virtual FS.
#include <emscripten/emscripten.h>
#ifdef WEB_DEBUG
  #define LOGD(...)  emscripten_log(EM_LOG_CONSOLE, __VA_ARGS__)
  #define LOGE(...)  emscripten_log(EM_LOG_CONSOLE | EM_LOG_ERROR, __VA_ARGS__)
#else
  #define LOGD(...)  ((void)0)
  #define LOGE(...)  emscripten_log(EM_LOG_CONSOLE | EM_LOG_ERROR, __VA_ARGS__)
#endif
...
// Placeholder type so existing struct fields referencing AAsset still compile.
// In the web build this pointer is always NULL — assets are accessed via
// regular POSIX file I/O over the Emscripten preloaded virtual filesystem.
typedef void AAsset;
// =====================================================================
...
struct _HANDLE {
    ...
    AAsset* fileAsset; // always NULL in web build (assets use fileDescriptor)
...
// Win32 multimedia (mmsystem.h) flags used by audio stubs.
#ifndef WHDR_DONE
...
// BSD sockets typedef (used by win32_ioctlsocket).
#ifndef u_long
