...
#ifdef __EMSCRIPTEN__
    {
        static int s_rs_called = 0;
        s_rs_called++;
        if (s_rs_called <= 10) {
            LOGD(
                "RomSwitch ENTRY #%d: adr=%u type=0x%02X pbyRom=%p dwRomSize=%u Bank_FF=%u",
                s_rs_called, (unsigned)adr,
                (BYTE)cCurrentRomType, pbyRom, dwRomSize, (unsigned)Chipset.Bank_FF);
        }
    }
#endif
	// only HP39/40G, HP49G
...
#ifdef __EMSCRIPTEN__
	{
		extern void __emscripten_log_formatted(int, const char*, ...);
		static int s_rs_ctr = 0;
		if (s_rs_ctr++ < 5) {
			LOGD(
				"RomSwitch #%d: adr_in=%u Bank_FF=%u type='%c' pbyRom=%p dwRomSize=%u",
				s_rs_ctr, (unsigned)adr, (unsigned)Chipset.Bank_FF, cCurrentRomType, pbyRom, dwRomSize);
			LOGD(
				"  pbyRomView[0]=%p (off=%ld)  pbyRomView[1]=%p (off=%ld)",
				pbyRomView[0], (long)((LPBYTE)pbyRomView[0] - pbyRom),
				pbyRomView[1], (long)((LPBYTE)pbyRomView[1] - pbyRom));
			if (pbyRomView[0]) {
				LOGD(
					"  pbyRomView[0][0..7] = %02X %02X %02X %02X %02X %02X %02X %02X",
					pbyRomView[0][0], pbyRomView[0][1], pbyRomView[0][2], pbyRomView[0][3],
					pbyRomView[0][4], pbyRomView[0][5], pbyRomView[0][6], pbyRomView[0][7]);
			}
			LOGD(
				"  RMap[0]=%p (off=%ld)  RMap[0][0..7] = %02X %02X %02X %02X %02X %02X %02X %02X",
				RMap[0], RMap[0] ? (long)(RMap[0] - pbyRom) : -1,
				RMap[0]?RMap[0][0]:0, RMap[0]?RMap[0][1]:0, RMap[0]?RMap[0][2]:0, RMap[0]?RMap[0][3]:0,
				RMap[0]?RMap[0][4]:0, RMap[0]?RMap[0][5]:0, RMap[0]?RMap[0][6]:0, RMap[0]?RMap[0][7]:0);
		}
	}
#endif
