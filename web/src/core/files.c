...
// === DEBUG ROM LOADING ===
	{
		...
		LOGD("MapRom DONE: dwFileSize=%u dwRomSize=%u bRomPacked=%d dwSize(first4)=0x%08X",
		     dwFileSize, dwRomSize, bRomPacked, dwSize);
		LOGD("  pbyRom[0..15] = %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X",
		     pbyRom[0],  pbyRom[1],  pbyRom[2],  pbyRom[3],
		     pbyRom[4],  pbyRom[5],  pbyRom[6],  pbyRom[7],
		     pbyRom[8],  pbyRom[9],  pbyRom[10], pbyRom[11],
		     pbyRom[12], pbyRom[13], pbyRom[14], pbyRom[15]);
		LOGD("  pbyRom[mid]   = %02X %02X %02X %02X (at offset %u)",
		     pbyRom[dwRomSize/2], pbyRom[dwRomSize/2+1], pbyRom[dwRomSize/2+2], pbyRom[dwRomSize/2+3], dwRomSize/2);
		LOGD("  pbyRom[end-4] = %02X %02X %02X %02X",
		     pbyRom[dwRomSize-4], pbyRom[dwRomSize-3], pbyRom[dwRomSize-2], pbyRom[dwRomSize-1]);
	}
...
BOOL NewDocument(VOID)
{
	...
	LOGD(
	    "files.c about to RomSwitch(0) for boot, type=0x%02X, pbyRom=%p",
