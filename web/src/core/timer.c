...
// Saturn timer2 with MSB set means "already overflowed past zero".
// On native Win32 this case is harmless because the worker CPU runs
// in parallel and the ROM eventually rewrites t2 before this matters.
// In our single-threaded web build the CPU is stuck in a tight loop
// and the unsigned interpretation of t2=0xFFFFFFxx as "wait 6 days"
// freezes the emulator. Treat it as "fire ASAP" so CheckT2 runs and
// the SRQ/INTR machinery can react.
if ((uDelay & 0x80000000) != 0)
{
bOutRange = FALSE;// natural expiry, not capped
uDelay = 1;// fire immediately
}
else
#endif
